...
public class CustomerServiceImpl implements ICustomersService {
    ...
    public CustomerDetailsDto fetchCustomerDetails(String mobileNumber, String correlationHeader) {
        ...
        if(fetchedCardDetails != null) customerDetailsDto.setCardsDto(fetchedCardDetails.getBody());
        if(fetchedLoanDetails != null) customerDetailsDto.setLoansDto(fetchedLoanDetails.getBody());
        if(fetchedCardDetails != null)
            customerDetailsDto.setCardsDto(fetchedCardDetails.getBody());
        if(fetchedLoanDetails != null)
